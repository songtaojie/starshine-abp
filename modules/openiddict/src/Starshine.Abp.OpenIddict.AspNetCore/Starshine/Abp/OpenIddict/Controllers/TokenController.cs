...
using Starshine.Abp.OpenIddict.ExtensionGrantTypes;
using Volo.Abp;
...
public partial class TokenController : AbpOpenIdDictControllerBase
{
    ...
    public virtual async Task<IActionResult> HandleAsync()
    {
        ...
        var extensionTokenGrant = extensionGrantsOptions.Value.Find<ITokenExtensionGrant>(request.GrantType ?? string.Empty);
